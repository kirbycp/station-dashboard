...
**Band conditions heatmap is an explicit heuristic, not real VOACAP.** `sunElevationDeg()` computes actual solar elevation for the station's location/hour; `bandLevel()` combines that with live SFI/K-index/X-ray class into a continuous 0–1 score per band/hour; `heatColor()` maps that to the red→amber→teal gradient. This is clearly labeled in the UI copy as a simplified model, not a certified ITU-R P.533/VOACAP run — a real VOACAP integration would need a WASM port or a backend service (out of scope for this single-file app; see the `VOACAP_SERVICE_URL` placeholder constant, currently unused, left as the intended integration point).
